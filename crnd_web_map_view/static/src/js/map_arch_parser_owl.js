...
export class MapArchParser extends XMLParser {
    parse(arch, models, modelName) {
        ...
        this.visitXML(arch, (node) => {
            switch (node.tagName) {
                case "crnd_map_view": {
                    ...
                    if (!node.hasAttribute("method_get_form_view_id")) {
                        throw new MapParseArchError(
                            `Map view has not defined "method_get_form_view_id" attribute.`
                        );
                    } else {
                        method_get_form_view_id = node.getAttribute(
                            "method_get_form_view_id"
                        );
                    }
                }
            }
        ...
        return {
            ...
            method_get_form_view_id: method_get_form_view_id,
        };
    }
}

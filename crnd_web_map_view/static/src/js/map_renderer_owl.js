...
export class MapRenderer extends Component {
    ...
    async get_map_api_key() {
        var api_key = await this.env.searchModel.orm.call(
            "ir.config_parameter",
            "get_param",
            [],
    ...
    get_map_data() {
        ...
        for (let x of Object.entries(this.props.model.data)) {
            res.push({
                id: x[1]["id"],
                lat: x[1][this.props.archInfo["latitude_field"]]
                    ? x[1][this.props.archInfo["latitude_field"]]
                    : 0,
                lng: x[1][this.props.archInfo["longitude_field"]]
                    ? x[1][this.props.archInfo["longitude_field"]]
                    : 0,
                title: x[1][this.props.archInfo["title_field"]]
                    ? x[1][this.props.archInfo["title_field"]]
                    : "",
            });
        }
        return res;
    ...
    async initMap() {
        ...
        this.markerCluster = new MarkerClusterer(this.map, this.markers, {
            imagePath: "/crnd_web_map_view/static/lib/markerclusterer/img/m",
            gridSize: 40,
            maxZoom: 7,
            zoomOnClick: true,
            zoomControl: true,
    ...
    load_markers() {
        ...
        for (let [id, position] of Object.entries(this.data)) {
            if (!this.isValidCoordinate(position["lat"], position["lng"])) {
                // Handle invalid coordinates
                console.error(
                    `Invalid coordinates for marker with ID ${position["id"]}`
                );
            ...
            marker.addListener("click", async (event) => {
                ...
                event.method_get_form_view_id =
                    this.props.archInfo["method_get_form_view_id"];
                self.env.searchModel.trigger("open_record", {
                    id: event.id,
                    method_get_form_view_id: event.method_get_form_view_id,
                });
            });
        ...
        console.log("Load " + this.markers.length + " Markers");
        this.markerCluster.addMarkers(this.markers);
    }

    isValidCoordinate(lat, lng) {
        // Add your validation logic here
        // For example, check if lat and lng are numbers within a valid range
